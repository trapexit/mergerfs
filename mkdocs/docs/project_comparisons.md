...
nofs is a lightweight userspace tool that provides mergerfs-like
functionality for combining multiple filesystems/directories into a
unified view, but does so entirely through subcommands rather than
mounting a FUSE filesystem. It takes a lot of inspiration from
mergerfs in that it supports policy-based branch selection (mfs, ff,
pfrd, epmfs, and others), read-only and no-create branch modes, and
recreates POSIX-like commands such as `ls`, `find`, `which`, `cp`,
`mv`, and `rm`.

The fundamental difference from mergerfs is that nofs does not mount a
filesystem. Instead of applications transparently accessing a unified
view, users interact with the pool explicitly via nofs subcommands.
This makes it well-suited for scripting and automation, but means
applications cannot use it as a standard mountpoint and therefore 3rd
party applications won't be able to see the pool. mergerfs provides a
transparent FUSE mountpoint which applications access using normal
POSIX filesystem semantics without any modification.

nofs is a good fit if you want policy-aware file operations and branch
management from scripts or the command line and do not need a
persistent mount. mergerfs is the better choice when you need
applications to see a unified filesystem without any changes to how
they access files.
...
are important to your workload, policyfs is worth evaluating alongside
mergerfs.
