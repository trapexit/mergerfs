                   ...
                   httplib::Response      &res_)
{
  ...
  branches = str::split(branches_str,':');
  ...
  for(const auto &branch_full : branches)
    {
      ...
      std::tie(path,mode) = str::splitkv(branch_full,'=');
                      ...
                      char      **argv_)
{
  ...
  rv = http_server.listen(host,port);
  if(!rv)
    {
      fmt::print(stderr,
                 "mergerfs.webui: error - failed to bind to {}:{} - {}\n",
                 host,
                 port,
                 std::strerror(errno));
      return EXIT_FAILURE;
